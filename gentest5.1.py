...
def encoder(parent, input):
    ...
    parent.kl_loss = kl_divergence(linear_output[0], linear_output[1]) * 0.001 + 0.1
...
def open_img(file_name):
    return preprocess_img(io_node, np.array(Image.open(file_name)))
...
def init_coders(image):
    input_dim = patch_size * patch_size * io_node.C
...
def train(input_img, desired_img=None, epochs=1000, lr=0.01):
    ...
    input_shaped = reconstruct_image(input_img,  io_node.H, io_node.W, io_node.patch_size, io_node.C)
    ...
    for epoch in range(epochs):
        new_img = reconstruct_image(io_node(input_img),  io_node.H, io_node.W, io_node.patch_size, io_node.C)
        ...
        #latent_div = kl_divergence(desired_img.flatten() * 0.001 + 0.1,
        #                           new_img.flatten() * 0.001 + 0.1)
        ...
        grad_enc_w, grad_enc_b = compute_gradients(total_loss, input_img, encoder_node.weight)
        grad_dec_w, grad_dec_b = compute_gradients(total_loss, latent_node.output, decoder_node.weight)

        encoder_node.weight *= enc_optimizer.update(grad_enc_w, grad_enc_b)
        #encoder_node.bias = enc_optimizer.update(input_img, grad_enc_b)
        decoder_node.weight *= dec_optimizer.update(grad_dec_w, grad_dec_b)
        #decoder_node.bias = dec_optimizer.update(latent_node.output, grad_dec_b)
...
io_node = Base({"name": "image reshaper", "patch_size": (patch_size, patch_size)}, train=False)
...
i/o - accepts input, preprocesses, passes to prediction network
prediction network - layers/neurons, etc
training network?
