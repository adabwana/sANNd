...
"""def reshape_image(input_img):
    \"""
    Process the input image through the reshaping network and reconstruct it.
    This returns an image that is cropped to full patches.
    \"""
    H, W, C = input_img.shape
    flat_patches = token_node.(input_img)
    # Reassemble patches into an image.
    reconstructed = reconstruct_image(flat_patches, H, W, patch_size, C)
...
def train(input_img, desired_img=None, epochs=1000, lr=0.01):
    ...
    input_shaped = reconstruct_image(tokenize_and_normalize_image(input_img, (patch_size, patch_size)),  H, W, patch_size, C)
